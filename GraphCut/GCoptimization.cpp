...
//	if (e00 + e11 > e01 + e10 && e00 < 1e5 && e11 < 1e5) {
//		printf("%lf %lf %lf %lf\n", e00, e11, e01, e10);
...
void GCoptimization::updateLabelingDataCosts()
{
	...
	for (int i = 0; i < m_num_sites; ++i) {
		...
		if (m_labelingDataCosts[i] > 1000) {
			printf("%d %d %lf\n", i, m_labeling[i], m_labelingDataCosts[i]);
			i = i;
		}
	}
}
...
GCoptimization::EnergyType GCoptimization::expansion(int max_num_iterations, int background)
{
	...
	try 
	{
		if ( max_num_iterations == -1 )
		{
			...
			do
			{
				...
				do 
				{
					if ( (background == next) ||  !alpha_expansion(m_labelTable[next]) )
