...
#include "NonRigidColorICP.h"
#include "ICP/ICP.h"

#define USE_DEPTH

class NonRigidColorICPError {
public:
  NonRigidColorICPError(cv::Mat& grey_img, cv::Mat& depth_img, double pt_color, ml::vec3f& point, double fx, double fy, double cx, double cy, int width, int height)
	: grey_img(grey_img), depth_img(depth_img), pt_color(pt_color), point(point), fx(fx), fy(fy), cx(cx), cy(cy), width(width), height(height)
  {}

  template <class T>
  bool operator()(const T* const R, const T* const tr, const T* const cl, T* residuals) const {
	T pt_w[3];
	for (int i = 0; i < 3; ++i){
	  pt_w[i] = (T)point[i];
	}
	...
	// nonrigid correct
	NonRigidAdjust<T>(x, y, cl, width, height);
	int px = *(double*)&x;
	int py = *(double*)&y;
	// test boundary
  ...
  static ceres::CostFunction* Create(cv::Mat& grey_img, cv::Mat& depth_img, double pt_color, ml::vec3f& point, double fx, double fy, double cx, double cy, int width, int height){
	...
	return (new ceres::AutoDiffCostFunction<NonRigidColorICPError, 1, 3, 3, 2 * (lattice_width + 1) * (lattice_height + 1)>(
	  new NonRigidColorICPError(grey_img, depth_img, pt_color, point, fx, fy, cx, cy, width, height)));
  }
private:
  cv::Mat& grey_img;
  cv::Mat& depth_img;
  ...
  for (int f = 0; f < kf.frames_.size(); f++)
  {
	...
	for (int v = 0; v < vts.size();)
	{
	  ...
	  problem.AddResidualBlock(
		NonRigidColorICPError::Create(kf.grey_images_[f], kf.depth_images_[f], (double)gm.at<unsigned char>(0, 0), mesh.m_Vertices[vts[v]], intrinsic(0, 0), intrinsic(1, 1), intrinsic(0, 2), intrinsic(1, 2), kf.grey_images_[f].cols, kf.grey_images_[f].rows),
		0, param, param + 3, kf.cls_[f].correction_
		);
	  v += 10;
	}
